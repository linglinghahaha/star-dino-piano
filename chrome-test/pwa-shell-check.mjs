...
try {
  ...
  record(
    ...
    shellMetadata.version === "app.js?v=overhaul-347a-c4-r01a" && shellMetadata.chapter4Css === "chapter4-slice.css?v=overhaul-347a-c4-r01a" && shellMetadata.mapVisible &&
  ...
  const cacheInventory = await page.evaluate(async () => {
    ...
    const cacheName = cacheNames.find((name) => name === "star-dino-pwa-overhaul-347a-c4-r01a") || "";
  ...
  const expectedChapter4CssPath = "/chapter4-slice.css?v=overhaul-347a-c4-r01a";
  const chapter4CssRuntimeEntries = runtimeShellPaths.filter((item) => item.includes("chapter4-slice.css"));
  record(
    "HTML, active runtime and precache share the LP03 Chapter 4 stylesheet identity",
    shellMetadata.chapter4Css === "chapter4-slice.css?v=overhaul-347a-c4-r01a" &&
  ...
  record(
    ...
    cacheInventory.controlled && cacheInventory.cacheName === "star-dino-pwa-overhaul-347a-c4-r01a" &&
