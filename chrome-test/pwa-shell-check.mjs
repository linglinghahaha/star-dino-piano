...
try {
  ...
  record(
    ...
    shellMetadata.version.includes("overhaul-341a") && shellMetadata.mapVisible &&
  ...
  const cacheInventory = await page.evaluate(async () => {
    ...
    const cacheName = cacheNames.find((name) => name === "star-dino-pwa-overhaul-341a-v1") || "";
  ...
  record(
    ...
    cacheInventory.controlled && cacheInventory.cacheName === "star-dino-pwa-overhaul-341a-v1" &&
