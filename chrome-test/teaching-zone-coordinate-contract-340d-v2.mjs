...
const outputPath = process.argv[3] || "docs/30_TEACHING_ZONE_COORDINATE_CONTRACT_340D_V2.json";
const screenshotPrefix = process.argv[4] || "screenshots/teaching_zones_340d_v2";
...
const contractSourceFiles = [
  "chrome-test/teaching-zone-coordinate-contract-340d-v2.mjs",
  "index.html",
  "app.js",
  "styles.css",
  "staff-overrides.css",
...
try {
  for (const spec of viewportSpecs) {
    ...
    await navigateReady(page, makeUrl(`?level=M03&check=zones-340d-v2-${spec.viewportId}`), ".moon-yard", `${spec.viewportId}:m03`);
    ...
    await navigateReady(page, makeUrl(`?mode=staff&check=zones-340d-v2-${spec.viewportId}`), ".staff-stage", `${spec.viewportId}:staff`);
...
const contract = {
  ...
  coordinateContractId: "teaching-zones-overhaul-340d-v2",
  contractSha256: null,
