...
const outputPath = process.argv[3] || "docs/30_CHAPTER3_LS05_MEDIA_ZONE_CONTRACT_341A_V1.json";
const screenshotDir = process.argv[4] || "screenshots/chapter3_ls05_media_zones_341a_v1";
...
const sourcePaths = [
  "chrome-test/chapter3-ls05-media-zone-contract-341a-v1.mjs",
  "index.html",
  "app.js",
  "styles.css",
...
function url(search = "?screen=map&check=chapter3-ls05-media-zones-341a-v1") {
...
function fixture() {
  return {
    version: 1,
    active: null,
    history: canonicalC1C2History({ completedAt: "2026-07-11T01:00:00.000Z", tag: "chapter3-ls05-zones" }),
    lastRest: null,
    chapter3: {
...
async function geometry(page, stateName, allowedPhases = expectedActualPhases[stateName], timeout = 12000) {
  return page.evaluate(async ({ stateName, margin, allowedPhases, timeout }) => {
    ...
    const candidates = rect("#ls05FlowerArc");
    ...
    return {
      ...
      zones: {
        ...
        threeFlowers: zone(candidates),
        replayControl: zone(replay),
        progress: zone(progress),
        repairFeedback: zone(["wrong-known", "pair-compare", "assisted-retry", "visual-assist"].includes(stateName) ? union(bubble, compare) : null),
...
try {
  for (const spec of viewports) {
    ...
    const referenceGeometry = geometry(page, "reference");
    await page.locator("#gardenRestMarker").click();
    states.push({ geometry: await referenceGeometry, screenshot: await maybeScreenshot(page, spec, "reference") });
    ...
    while ((active = await attempt(page))?.callIndex < 5) {
      await phase(page, "awaiting-response");
      active = await attempt(page);
      const completeObserved = active.callIndex === 4 ? geometry(page, "complete") : null;
      await press(page, active.sequence[active.callIndex]);
      if (completeObserved) {
...
for (const viewport of records) {
  ...
  for (const state of viewport.states) {
    ...
    if (geometry.state !== "garden-entry") {
      for (const required of ["keyboard", "xingyaAndBubble", "neutralSoundSource", "threeFlowers", "progress"]) {
      ...
      if (["pair-compare", "assisted-retry", "visual-assist"].includes(geometry.state) && !geometry.zones.repairFeedback.rect) {
...
const contract = {
  ...
  coordinateContractId: "chapter3-ls05-media-zones-overhaul-341a-v1",
  contractSha256: null,
  contractHashScope: "sha256 of canonical JSON with contractSha256 and generatedAt set to null",
  prototypeBaseline: "overhaul-341a",
  ...
  stateCoverage: expectedStates.map((state) => ({
    ...
    behaviorEvidence: "check:chapter3-ls05"
  })),
