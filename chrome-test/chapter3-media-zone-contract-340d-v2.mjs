...
const outputPath = process.argv[3] || "docs/30_CHAPTER3_MEDIA_ZONE_CONTRACT_340D_V2.json";
const screenshotDir = process.argv[4] || "screenshots/chapter3_media_zones_340d_v2";
...
const sourcePaths = [
  "chrome-test/chapter3-media-zone-contract-340d-v2.mjs",
  "index.html",
  "app.js",
  "styles.css",
...
function url(search = "?screen=map&check=chapter3-media-zones-340d-v2") {
...
try {
  for (const spec of viewports) {
    ...
    while ((active = await attempt(page))?.callIndex < 4) {
      await phase(page, "awaiting-response");
      active = await attempt(page);
      const completeObserved = active.callIndex === 3 ? phase(page, "complete") : null;
      await press(page, active.sequence[active.callIndex]);
      if (completeObserved) {
...
const contract = {
  ...
  coordinateContractId: "chapter3-media-zones-overhaul-340d-v2",
  contractSha256: null,
