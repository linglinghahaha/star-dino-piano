...
const outputPath = process.argv[3] || "docs/30_TEACHING_ZONE_COORDINATE_CONTRACT_344A_V1.json";
const screenshotDir = process.argv[4] || "screenshots/teaching_zones_344a_v1";
const clearanceMarginPx = 24;
const alphaThreshold = 8;
const navigationTimeoutMs = 30000;
const bootTimeoutMs = 30000;
const surfaceTimeoutMs = 20000;
...
const contractSourceFiles = [
  "chrome-test/teaching-zone-coordinate-contract-344a-v1.mjs",
  "index.html",
  "app.js",
  "styles.css",
  "staff-overrides.css",
...
async function navigationDiagnostics(page, stage, targetUrl, selector) {
  const fallback = { stage, targetUrl, selector, pageUrl: page.url(), pageStateUnavailable: true };
  try {
    return await page.evaluate(async ({ stage, targetUrl, selector }) => {
    ...
    }, { stage, targetUrl, selector });
  } catch (error) {
    return { ...fallback, diagnosticError: error?.message || String(error) };
  }
}

async function navigateReady(page, targetUrl, selector, stage) {
  try {
    ...
    await page.waitForSelector(selector, { state: "visible", timeout: surfaceTimeoutMs });
    await page.waitForTimeout(320);
  } catch (error) {
    const diagnostics = await navigationDiagnostics(page, stage, targetUrl, selector);
...
try {
  for (const spec of viewportSpecs) {
    ...
    await navigateReady(page, makeUrl(`?level=M03&check=zones-344a-v1-${spec.viewportId}`), ".moon-yard", `${spec.viewportId}:m03`);
    ...
    const workshopScreenshot = path.join(screenshotDir, `${spec.viewportId}_M03_initial.png`);
    ...
    const workshopWrongScreenshot = path.join(screenshotDir, `${spec.viewportId}_M03_wrong.png`);
    await page.screenshot({ path: workshopWrongScreenshot, fullPage: false });

    await navigateReady(page, makeUrl(`?mode=staff&check=zones-344a-v1-${spec.viewportId}`), ".staff-stage", `${spec.viewportId}:staff`);
    const staffInitial = normalizeGeometry(await readGeometry(page, "staff"));
    const staffScreenshot = path.join(screenshotDir, `${spec.viewportId}_S01_initial.png`);
    ...
    const staffWrongScreenshot = path.join(screenshotDir, `${spec.viewportId}_S01_wrong.png`);
...
const contract = {
  ...
  coordinateContractId: "teaching-zones-overhaul-344a-v1",
  contractSha256: null,
