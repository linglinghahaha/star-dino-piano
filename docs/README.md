...
7. `C4-02 / LP03` is frozen at `cedc384`: it wakes the existing C anchor, adds D/E and saves only a `C3-E3` stage observation. Independently freeze `C4-R01A` under `docs/48` next before starting LP04; the LP04 candidate must complete `C4-R01B` against real C4-03 actions before either milestone is called end-to-end complete. The sampled `LP07 -> LP08-mini -> LP10-mini` route remains an external observation plan, not permission to skip LP04-LP06 or reorder runtime delivery.
