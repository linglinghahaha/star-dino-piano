...
- Chapter 3 `LS01-LS08`, teaching-audio lifecycle batches A-C, Chapter 4 `LP01-LP03`, the isolated `C4-R01A` scheduler core, and their named regressions are complete at the committed baseline. The current unapproved browser/PWA experience candidate is `overhaul-369d-course-route-monotonicity-correction` / `app.js F3B4F960...B31F`; 369c is its direct stage-settlement/PWA predecessor, 369b its Figma A settlement source, 369a its staff-notation source, 368b its S01-continuity source, 367b its portable-recovery predecessor, and 366a its teaching-core predecessor. It preserves the accepted LP04 E-D-C loop, complete real-C4-03 R01B lifecycle and compact phase-tail settlement while changing only formal progression-history eligibility and forward garden-route recovery; it does not implement LP08+ bass staff or change lesson order, error thresholds or mastery semantics. The separate `overhaul-359a-map-shell-scroll-reset` / `app.js 5AB01914...E566` package remains the only frozen adult physical-iPad preflight and directional child-observation baseline.
- `C4-R01B` is supervisor-accepted in the uncommitted candidate: lifecycle `31/31`, foundation `20/20`, and independent exact-anchor adversarial probe `9/9`. Missing, time-mismatched or duplicate selected stable anchors fail closed without replacement stable/retained evidence.
- Curriculum/story coherence runs inside `check:quick`: all 39 canonical lessons retain opening/action/success/repair/learning sections; C4-R01 remains a non-node, same-session review without a score page; LP04 retains the E-D-C no-leak loop; LP05 locks E3/F3/G3 geography and dispatch gates; proposed C3-X01 remains teacher-gated; TH01-TH04 separate role/register teaching, hand evidence and staff landmarks; TH05 keeps five child-started 2/4/4/5/5-input actions and a TH08-only exit; every child completes TH08 core relay before any later TH06/TH07/together-encore invitation. The next gate expansion also rejects stale pre-finale optional text and keeps the `docs/71` naming-carrier proposal out of runtime until teacher review.
- The executable mature-app ledger distinguishes `check:maturity-state` from `check:maturity-release`. The compliant external UI result and supervisor triage close one former blocker, while the independently identified naming-carrier gap is now an explicit requirement. The user has also explicitly placed A-G word typing after the first mature release, so scope decision is passed while the future mode remains parked. Current-state facts must pass while strict release fails with 13 named blockers. A green Web quick gate still cannot be used as the mature-app completion claim.
- Frozen Web packaging now uses the self-contained 359a 47-file manifest in `docs/30_OBSERVATION_CANDIDATE_359A.json`; 354b-358a remain historical freeze evidence. The 354a ZIP pair is preserved as historical evidence but retired after the supervisor found four directly referenced CSS files missing from its 42-file policy. This closes only the Web audit-archive sub-item; signed native release and real rollback remain missing.
- The product-name fact source correction remains complete. Current experience candidate 367b preserves the parent-only version label, browser hidden/pagehide microphone release, exact parent reset, `UI-P0A/P0B/P0C`, map-shell origin reset and the frozen input-routing corrections; its Web parental-challenge reference remains browser-only and unapproved. 367a and 366a retain visual/teaching-core predecessor roles, while 365b retains only frozen static/source recovery ownership. The next adult physical-iPad preflight is still bound to the exact frozen 359a observation package, not to the live 367b workspace. LP04/R01B may not be reimplemented, and LP05, C3-X01, parent replay B/C and media remain forbidden unless separately dispatched.
- Garden-character v1/v2 remain rejected. The user-selected v3 has completed exact-hash 352b prototype runtime/PWA integration and independent browser verification; runtime writers are now frozen and release clearance remains false.
- `docs/62` accounts for the same 27 runtime images and `1,700,918` bytes in 359a. Traceability is complete as an inventory, not as release provenance: 7 assets have a reproducible pipeline, 1 exact candidate still lacks upstream rights, 18 early assets have summary-only source records, and the M08 background now has its exact prompt and runtime derivation recovered from the source thread while exact provider/model, generation ID, operation-specific terms receipt, dimension reconciliation, release rights and external similarity remain missing. The former M08 T5 character has been replaced by the reproducible A2 crop, but release rights and external similarity remain uncleared. Release clearance is `0/27`.
- `LP05+`, Chapter 5 and native migration remain locked. The external course ZIP is triaged under `docs/56`. The UI original stays quarantined, while its exact-source Markdown-only derivative passes strict intake and is triaged under `docs/65`; accepted P0/P1 items may now drive only their named narrow dispatches. Four concept PNGs remain excluded. Grok video generation remains hard-paused after user-confirmed quota exhaustion.
- `docs/66` is now the human-readable LP05 acceptance contract, not a runtime dispatch. LP05 stays forbidden until the exact 359a adult physical-iPad preflight passes, the first directional child observation is reviewed without a blocking P0, and the supervisor issues a separate `C4-04 / LP05 only` dispatch; the idle prototype task is not contacted merely because the checklist exists.
- `docs/67` now locks the future TH01-TH02 role/register teaching bundle, correct `低音 G / 上面的 G` child language, pair-level rests, any-hand validity, later-review evidence and no C5-01 settlement. It is not a runtime dispatch: Chapter 5 stays forbidden until LP10 runtime/story exit, teacher review and a separate narrow supervisor dispatch pass.
- `docs/68` now locks future TH03-TH04: first-play TH03 is one free-time `C3-C4-D3-D4-E3-E4` role/register route with discrete releases and C/D/E pair rests; hand icons invite but never prove hand use. TH04 is two one-at-a-time exact staff-to-key C landmarks and the first Chapter 5 phase tail. It is not a runtime dispatch.
- `docs/69` now locks future TH05: one public mission contains supports, two separate high bars and two separate relay bars; each requires a new child click, retains every permanent step, writes component played/order only, and reaches one clean-tail phase milestone before a single TH08 exit. It is not a runtime dispatch.
- `docs/70` closes the TH06-TH08 route-order contradiction: core TH08 opens the garden first; later TH06 uses same-name C/C and G/G pairs before TH07 transfers to the real bar starts, and together-encore cannot rewrite story completion or reward.
- `docs/71` closes the planning ambiguity around `音符护照`: collection remains non-mastery, while proposed `NP-CDE/NP-FG` independently checks dinosaur-solfege to letter-name retrieval and keeps the final guided key action as separate evidence. It awaits preschool-teacher review; runtime remains frozen and the prototype task is not contacted merely because the checklist exists.
...
Status: `C4-01 / LP01-LP02`, `C4-02 / LP03`, and the separate `C4-R01A` scheduler core are independently accepted in `overhaul-347a-c4-r01a` / `af9aa28` under `docs/46-49`. The current unapproved browser/PWA experience candidate is `overhaul-369d-course-route-monotonicity-correction` / `app.js F3B4F960...B31F`. It preserves the independently accepted LP04 loop and complete real-C4-03 R01B lifecycle: LP04 `28/28 + 8/8 + 11/11`, R01B `31/31`, foundation `20/20`, retained-anchor adversarial probe `9/9`; it also preserves browser microphone background release, parent reset, UI-P0A/P0B/P0C, compact phase-tail settlement and map-shell origin behavior. 369c is its direct stage-settlement/PWA predecessor, 369b its Figma A settlement source, 369a its staff-notation source, 368b its S01-continuity source, 367b its portable-recovery predecessor, and 366a its teaching-core predecessor. The next gate remains adult physical-iPad audio preflight against the separately frozen 359a observation package, not another LP04/R01B implementation and not an automatic promotion of 369d. `LP05+`, bass staff, left hand, full Dongdong reveal and broader media integration remain locked pending an explicit post-observation dispatch.
