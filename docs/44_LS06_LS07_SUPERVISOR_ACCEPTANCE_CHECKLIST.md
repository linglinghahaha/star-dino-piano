...
状态：`draft_ready / LS05_gate_waiting / runtime_locked`

## 一、用途与解锁条件

本文件预先锁定第三章后续两个听辨里程碑，避免 `LS05` 通过后临时拼玩法。它不是当前运行工作单，也不授权原型任务实现 `LS06+`。

只有同时满足以下条件，主管才可把本草案晋升为正式工作单：
...
- 家长区和不泄露目标的键盘 ARIA 可保留双身份；隐藏目标不得进入 `aria-label`、`title`、`alt`、class、data、伪元素或隐藏说明。
...
- `passed_spec_draft`：故事、教学轴、短课节奏、证据和验收边界已形成可审查草案。
- `locked`：LS05 尚未由主管独立通过，因此不得派发 LS06-LS07 运行实现。
