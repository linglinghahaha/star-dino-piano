...
状态：`specification_passed / overhaul-342a_ls01_ls07_browser_passed / ls08_runtime_dispatched / media_not_approved / device_and_child_evidence_missing`
...
- 除星芽口语/气泡外，键盘、任务、谱垫、路线、反馈、结果、地图、花园物件及其孩子端 ARIA 只显示 C/D/E/F/G；同一屏不重复堆叠多个等价身份卡，也不显示或朗读普通 `Do/C` 双标。
