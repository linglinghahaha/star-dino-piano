...
2026-07-11 第三章合同交叉审查补充：`docs/03/09/17/24/32/33` 已统一 `C3-01` 的提前休息条件，LS01 出现 repeated repair、strong/assisted cue、modeled success、长等待或疲劳时就在第一片叶保持打开处结束，并在下次只续 LS02；自然停点不得用叶片重新合拢制造进度撤销感。原 `LS08 星芽唱两声` 已改为 `LS08 根须记两声`，目标由中央中性声源播放，星芽只做同态“听两声”动作；稳定检查不得分音重听，整轮最多一次孩子主动整组重听，超出仍给故事结果但不写 stable。以上为 `specification_passed / runtime_missing`，只进入未来 Chapter 3 启动包，不发送给当前 M08 active turn。
