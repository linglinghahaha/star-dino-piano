...
- 冻结提交：`af9aa2831bb90c94fd10fdfbe3ca89ba4d03021a`；运行身份：`overhaul-347a-c4-r01a`；最终 `app.js` SHA-256：`5E2041D554E20DA7E98A5C0544B1834051B4DD8AA7D684388F8A43B0CCB0D4DE`。提交只有 R01A 运行核心、隔离真实界面夹具、专项/主管门禁、三轮完整合同、版本/缓存断言和 `docs/20`；没有探针合同、旧漂移合同、媒体运行手册、Grok/Gemini 候选、LP04 或 C4-03 正式 session。
- 主管在冻结源码上独立复跑：scheduler `35/35`、fixed supervisor evidence `19/19`、P1 adversarial `29/29`、R01A audio lifecycle `12/12`、artifact authenticity `20/20`、triple-contract authenticity `19/19`、`check:quick` 与 strict bundle 全部退出 0。共享门禁由同一冻结摘要保持：课程故事 `29/29`、孩子端音名 `276/276`、session `74/74`、clean-state `124/124` 以及第一至第三章、LP01-LP03、音频、PWA、输入、iPad a11y、布局和装备回归均通过。
- 三轮合同各为 6 个横向浏览器视口 x 7 个真实调度状态 = 42 PNG、0 failure、0 browser error；三轮 core SHA 均为 `32b8f7b84a9d25e5a3fa7d43c1c0b6ff5f343552f509dcb92d68c74aaa492401`，42 个逐状态 PNG 哈希跨轮完全一致。规范合同与第三轮字节一致，证据不是旧 344a 正课合同的改名副本。
- 教学裁决：R01A 只在 LP01 高低 C 听辨和 LP02 精确低音 C 键位两项已学技能之间公平选择；候选身份严格限定为 `level:LP01 -> LP01` 与 `low-key:C3 -> LP02`，未知、缺失和错配记录全部 fail-closed。任一复习后至少留出一个真实无复习故事 session；resume 永远 story-first；困难只写补练和冷却，不删除既有 stable/retained，也不把系统、MIDI 观察或麦克风歧义冒充稳定掌握。
- 裁决：`C4_R01A_core_passed / browser_baseline_promoted / C4_R01B_still_required / LP04_dispatch_ready / media_locked / release_external_evidence_missing`。`docs/48` 的 A 门禁转为已接受基线；`docs/51` 的 LP04 + C4-R01B 成为下一唯一运行工作，必须单独提交并使用真实 C4-03 actions 证明同 session 继续、困难收短、刷新恢复与 story-first 间隔。B 通过前不得称 C4-R01 完整运行通过，也不得开始 LP05。

## 2026-07-19 `overhaul-347a-c4-r01a` 首候选退回与证据完整性 P1

- 主管以当前工作树而非支线自报为准复核：候选基于 `9f5d2b83ddeb97bbee9a860cebdfdb14f1cbb14d`，当前 `app.js` SHA-256 为 `10ED23E28F08AB92DC39C0C778892BFA76FC52A367F38FFB7EF5002FA0F27644`。独立端口上的 `check:chapter4-r01a` 确实为 `22/22`，`check:quick` 与 strict bundle 也通过，运行包仍为 42 个文件、1,641,265 bytes；这些结果只证明一组 opt-in 纯函数断言和既有静态门禁，不证明 C4-R01A 生命周期通过。
- 当前 `chrome-test/chapter4-r01a-media-zone-contract-347a-v1.mjs` 只是包装旧 `chapter4-lp01-lp02-media-zone-contract-344a-v1.mjs` 并替换合同 ID。生成 JSON 仍明确写 `prototypeBaseline=overhaul-344a`、`buildIdentity=overhaul-344a-p3`、源 runner 为旧 344a 文件，19 个状态也是 LP01/LP02 正课状态；因此 6 视口 x 19 状态、114 张图和 internal SHA `3e8eedc785bf2f076705128622c218720d6cc3db6026b307d17acc947a0adcb9` 全部标为 `rejected_not_R01A_evidence`，不得重签、晋升或进入新基线。
- 当前 `app.js` 只新增迁移、选择、compose、classify、settle 纯函数及测试专用 API，没有 C4-R01A 的隔离 review 状态机。暂停、刷新、PWA/崩溃恢复、LP01 remediation/reduced-cue 四次调用、LP02 child audio ended 后结算、困难安全收短和幂等 settle 均无真实证据；正式地图没有创建空 C4-03 是正确边界，但不能用“未接孩子路径”替代 A 阶段规定的专用内存/隔离命名空间生命周期夹具。
- 主管对抗性浏览器探针复现三项实际误判：只有 `{role:"lesson"}`、没有 presented/completed 或孩子参与的空 ended session 会同时解除全局 review 间隔和候选 cooldown；真实第四章运行使用的输入路由 `屏幕` 在精确 MIDI 48 上得到 `stableEligible=false`，测试自造的英文 `screen` 却得到 true；缺 `eventId/sessionId/completedAt/localDateKey` 的伪 stable 事件仍会生成 `retained-due` 候选。另一个有效旧 stable event 已跨日超过 8 小时，但同日普通 formal play 更新了 `lastFormalCompletedAt` 后，当前算法反而不再生成到期候选，违反“普通练习不得重置合格 anchor”的合同。
- 当前所谓 storage-byte 断言在 app 已加载和迁移后才读取空/既改写的 localStorage，再只调用纯函数比较前后，不能证明启动迁移、隔离 review、刷新和恢复不污染非空正式 history/evidence/stable/retained/world。当前 scheduler 专项也没有教学音 started/ended、mute/volume 0、suspend、resume reject、watchdog、导航中断或恢复，却被加入 `tools/audio-lifecycle-suite.mjs`；这属于证据外推，必须移出并由独立真实 R01A audio lifecycle 门禁替代。
- 主管新增永久对抗门禁 `chrome-test/supervisor-chapter4-r01a-evidence-check.mjs`，不进入普通 quick，也不冒充完整音频套件。它在独立端口对当前草稿得到 `8/19`：旧 runner/旧合同身份、R01A 状态元数据、专用音频门禁、空故事/review-only 冷却、真实 `屏幕` 路由、无效 stable anchor 和有效旧 anchor 不被普通 play 推迟共 11 项按预期失败；direct 不消费、真实孩子故事消费、MIDI/mic 分轴、reduced-cue plan 幂等和三项非空正式存储迁移保护通过。该结果把退回原因变成可重复门禁，同时证明无需推翻已经正确的启动迁移数据保护。
- 裁决：`rejected_as_C4_R01A_baseline / narrow_pure_functions_partial / old_contract_invalid / state_machine_audio_storage_evidence_missing / LP04_locked`。修正版必须真实建立隔离 review 状态机和非空正式存储保护，要求实际 presented 即写全局间隔，只有后来正常结束且有真实孩子参与的新故事 session 才消费间隔；使用现有运行输入词汇；以有效 stable event 的时间/session/本地日期为 anchor，并正确处理只有 v2 `stableCompletions` 的首次 review 只补可追溯 stable、不得关闭 retained 候选。随后另建真实 R01A 音频门禁和真实六视口合同三连，再跑全部共享回归。此前不得派发 LP04、C4-R01B 或媒体接入。
...
| `NEXT-NATIVE` | 未来独立原生任务 | 按 `docs/36` 依次做 N0 平台探针、N1 教学纵切、N2 五章迁移、N3 TestFlight/商店 | `spec_ready / web_reference_ready_at_347a_r01a / task_not_created / locked_by_mac_xcode_account_and_physical_ipad` |
